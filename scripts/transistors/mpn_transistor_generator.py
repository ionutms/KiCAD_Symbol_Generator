...
Note:
Requires predefined series specifications in symbol_diode_specs.SYMBOL_SPECS
...
def generate_files_for_series(
    ...
    Args:
        series_name: Series identifier (must exist in SYMBOL_SPECS)
        unified_parts_list: List to append generated parts to

    Raises:
        ValueError: If series_name is not found in SYMBOL_SPECS
    ...
    """
    if series_name not in symbol_transistor_specs.SYMBOL_SPECS:
        msg = f"Unknown series: {series_name}"
        raise ValueError(msg)

    specs = symbol_transistor_specs.SYMBOL_SPECS[series_name]
...
if __name__ == "__main__":
    try:
        ...
        for series in symbol_transistor_specs.SYMBOL_SPECS:
