...
def create_description(
    inductance: float,
    specs: symbol_coupled_inductors_specs.SeriesSpec, is_aec: bool,  # noqa: FBT001
) -> str:
...
def generate_files_for_series(
    ...
    """
    if series_name not in symbol_coupled_inductors_specs.SERIES_SPECS:
        msg = f"Unknown series: {series_name}"
        raise ValueError(msg)

    specs = symbol_coupled_inductors_specs.SERIES_SPECS[series_name]
...
if __name__ == "__main__":
    try:
        ...
        for series in symbol_coupled_inductors_specs.SERIES_SPECS:
